#!/usr/bin/sh
while getopts u:x:a:s:e:t:f:g: flag
do
    case "${flag}" in
        ...
        e) end=${OPTARG};;
        t) step=${OPTARG};;
        f) queries_file=${OPTARG};;
        g) generate_graphs=${OPTARG};;
        *)
    esac
done

sudo mv ${queries_file} ./script
...
sudo docker run --cidfile /tmp/prometheus_exporter.cid -e prometheus_base_url="${prometheus_base_url}" \
       ...
       -e queries_file="${queries_file}" -e generate_graphs="${generate_graphs:-"False"}" prometheus_chart_exporter:0.0.1
