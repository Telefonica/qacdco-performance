...
echo "+++ Upload data to QA Reporter"
...
if (( ${#PROJECT_OBJECT} )); then
  ...
  EXECUTION_OBJECT=$(curl -X POST -d "project-id=${PROJECT_ID}&project-version=${PERFORMANCE_VERSION_NUMBER}&execution-name=autoexecution_${PERFORMANCE_VERSION_NUMBER}&execution-date=${EXECUTION_DATE}&module=${PERFORMANCE_PROJECT_MODULE}" ${QA_REPORTER_URL}/api/1.0/performance/executions/)
