...
# NON-UI JMeter execution mode using remote node injector (JMeter slave node)
if [ -z "$PERFORMANCE_ENGINE"  ]; then
  PERFORMANCE_ENGINE="jmeter"
fi
...
if (( ${#PROJECT_OBJECT} )); then
  ...
  curl -X POST -F "module=${PERFORMANCE_PROJECT_MODULE}" -F "data=@${PERFORMANCE_ENGINE}/samples.csv" -F "project-id=${PROJECT_ID}" -F "execution-id=${EXECUTION_ID}" -F 'input-type=jmeter-csv' ${QA_REPORTER_URL}/api/1.0/performance/csv_loader
